...
export const useUserStore = defineStore({
  ...
  actions: {
        ...
        .then(async () => {
          await signInWithEmailAndPassword(auth, email, password).then(
            async (res) => {
              ...
              await this.getProfileImage(user.uid).then((res) => {
                if (res) {
                  this.user["profile_image"] = res;
                }
              });
            }
          );
    ...
    async removeVehicle(id) {
      const newVehicle = this.user["vehicles"].filter(
    ...
    async removeContact(id) {
      const newContacts = this.user["contacts"].filter(
        (element, index) => index !== id
      );
    ...
    async updateSeen(id) {
      const newNotifications = this.user["notifications"].map(
        (element, index) => {
          if (index == id) {
    ...
    async updateProfile(data) {
      return await updateDoc(doc(db, "users", auth.currentUser.uid), {
        ...
        .then(async (res) => {
          await uploadBytes(
            ref(getStorage(), auth.currentUser.uid),
            data["profile_image"]
          ).then(async (res) => {
            this.user["profile_image"] = await this.getProfileImage(
